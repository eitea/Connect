        ...
        if (isset($_POST['editDynamicProject'])) { //new projects
            ...
            if (isset($available_companies[1]) && !empty($_POST['name']) && !empty($_POST['description']) && !empty($_POST['owner']) && test_Date($_POST['start'], 'Y-m-d') && !empty($_POST['employees'])) {
                ...
                if (!empty($_POST['editDynamicProject'])) { //existing
                    ...
                    echo $conn->error; //fk does the rest
                ...
                if (!empty($_POST['projecttags'])) {
                    $tags = implode(',', array_map(function($data) {
                                return preg_replace("/[^A-Za-z0-9]/", '', $data);
                            }, $_POST['projecttags'])); //strictly map and implode the tags
                } else {
                    $tags = '';
                }
                ...
                if (!$stmt->error) {
                    $stmt->close();
                    //EMPLOYEES
                    $stmt = $conn->prepare("INSERT INTO dynamicprojectsemployees (projectid, userid, position) VALUES ('$id', ?, ?)");
                    echo $conn->error;
...
<table class="table table-hover">
    ...
    <tbody>
        ...
        if ($filterings['priority'] > 0) {
            $query_filter = $query_filter . " AND d.projectpriority = " . $filterings['priority'];
        }
        ...
        while ($result && ($row = $result->fetch_assoc())) {
            ...
            if (($viewed = $viewed_result->fetch_assoc()) && $viewed['activity'] != 'VIEWED') {
            ...
            echo '<td>';
            if ($useRow = $isInUse->fetch_assoc()) {
                echo 'WORKING<br><small>' . $userID_toName[$useRow['userID']] . '</small>';
            } else {
                echo $row['projectstatus'];
            }
            if ($row['projectstatus'] != 'COMPLETED') {
                echo ' (' . $row['projectpercentage'] . '%)';
            }
            echo '</td>';
