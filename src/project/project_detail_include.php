<?php
if(!function_exists('insert_access_user')){
	...
	function drawFolder($parent_structure, $visibility = true){
		...
		while($result && ($row = $result->fetch_assoc())){
			...
			if($row['type'] == 'folder'){
				...
				<div class="col-xs-4"><a class="folder-structure.'.$projectID.'" data-child="'.$row['id'].'" data-parent="'.$parent_structure.'" >'.$row['name'].'</a></div><div class="col-xs-4">'.$row['uploadDate'].'</div>';
				...
				<div class="col-xs-3">
				<form method="POST" style="display:inline"><button type="submit" class="btn btn-default" name="delete-file-'.$projectID.'" value="'.$row['uniqID'].'">
...
$link_id = (getenv('IS_CONTAINER') || isset($_SERVER['IS_CONTAINER'])) ? substr($servername, 0, 8) : $identifier; //sync with download_archiveObject.php
$bucket = $link_id .'-uploads'; //no uppercase, no underscores, no ending dashes, no adjacent special chars
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	if(isset($_POST['saveGeneral_'.$projectID])){
		$hours = $projectRow['hours'] = floatval(test_input($_POST['project_hours']));
		$hourlyPrice = $projectRow['hourlyPrice'] = floatval(test_input($_POST['project_hourlyPrice']));
		$status = $projectRow['status'] = isset($_POST['project_productive']) ? 'checked' : '';
		$field_1 = $field_2 = $field_3 = 'FALSE';
		if(isset($_POST['project_field_1'])){ $field_1 = $projectRow['field_1'] = 'TRUE'; }
		if(isset($_POST['project_field_2'])){ $field_2 = $projectRow['field_2'] = 'TRUE'; }
		if(isset($_POST['project_field_3'])){ $field_3 = $projectRow['field_3'] = 'TRUE'; }
    ...
    } elseif(isset($_POST['hire_'.$projectID])){
    ...
    } elseif(!empty($_POST['add-new-folder-'.$projectID])){
        $x = test_input($_POST['add-new-folder-'.$projectID]);
    ...
    if(!empty($_POST['delete-file-'.$projectID])){
        $x = test_input($_POST['delete-file-'.$projectID]);
        try{
    ...
    if(!empty($_POST['add-new-file-'.$projectID]) && isset($_FILES['new-file-upload'])){
        ...
        if (!in_array($ext, ['doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'pdf', 'txt', 'zip'])){
            ...
            $parent = test_input($_POST['add-new-file-'.$projectID]);
            try{
    ...
    } elseif(!empty($_POST['add-new-file-'.$projectID])){
	    ...
	    <div class="page-header">
	        <h3><?php echo $projectRow['clientName'].' - '.$projectRow['name']; ?>
	            <div class="page-header-button-group">
	                <button type="submit" name="saveGeneral_<?php echo $projectID; ?>" class="btn btn-default" title="<?php echo $lang['SAVE']; ?>" ><i class="fa fa-floppy-o"></i></button>
	                <button type="submit" name="reKey_<?php echo $projectID; ?>" class="btn btn-default" title="Neues Schlüsselpaar erstellen" ><i class="fa fa-lock"></i></button>
	            </div>
	        </h3>
	    </div>
...
<?php if(isset($project_private)): ?>
	        ...
	        <div class="modal-dialog modal-content modal-md">
	            <form method="POST">
	                ...
	                <div class="modal-footer">
	                    ...
	                    <button type="submit" class="btn btn-warning" name="hire_<?php echo $projectID; ?>"><?php echo $lang['ADD']; ?></button>
	                </div>
	            </form>
	        </div>
	    </div>
	...
	<?php if(!empty($s3)) : ?>
		...
		<div id="modal-new-folder-<?php echo $projectID; ?>" class="modal fade">
			<div class="modal-dialog modal-content modal-sm">
				<form method="POST">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
						<button type="submit" class="btn btn-warning modal-new-<?php echo $projectID; ?>" name="add-new-folder-<?php echo $projectID; ?>" value="ROOT"><?php echo $lang['ADD']; ?></button>
					</div>
				</form>
			</div>
		</div>
		<div id="modal-new-file-<?php echo $projectID; ?>" class="modal fade">
			<div class="modal-dialog modal-content modal-sm">
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
						<button type="submit" class="btn btn-warning modal-new-<?php echo $projectID; ?>" name="add-new-file-<?php echo $projectID; ?>" value="ROOT"><?php echo $lang['ADD']; ?></button>
					</div>
				</form>
			</div>
		</div>
		...
		$('.tree-node-back-<?php echo $projectID; ?>').click(function(){
			var grandPa = grandParent.pop();
			$('#folder-<?php echo $projectID; ?>-'+ $(this).data('parent')).hide();
			$('#folder-<?php echo $projectID; ?>-'+ grandPa).fadeIn();
			$('.modal-new-<?php echo $projectID; ?>').val(grandPa);
		});
		$('.folder-structure-<?php echo $projectID; ?>').click(function(event){
			$('#folder-<?php echo $projectID; ?>-'+ $(this).data('parent')).hide();
			$('#folder-<?php echo $projectID; ?>-'+ $(this).data('child')).fadeIn();
			grandParent.push($(this).data('parent'));
			$('.modal-new-<?php echo $projectID; ?>').val($(this).data('child'));
		});
		</script>
	<?php else: ?>
		<h4>Dateifreigabe</h4>
