...
$pdf->SetFontSize(10);
$prod_res = $conn->query("SELECT products.*, (quantity * price) AS total, percentage AS taxPercentage FROM products, taxRates WHERE taxRates.id = taxID AND proposalID = ".$row['proposalID'] .' ORDER BY position ASC');
echo $conn->error;
