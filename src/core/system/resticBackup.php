...
<?php
$resticDir = dirname(dirname(dirname(__DIR__))) . "/plugins/restic/";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["restore"], $_POST["snapshot"])) {
        ...
        if (in_array("database", $tags)) { //Database backup
            ...
            $connectFolder = dirname(dirname(dirname(dirname(__DIR__))));
            set_time_limit(600);
            $connectFolder = escapeshellarg($connectFolder);
...
<?php if ($output ?? false): ?>
                ...
                <textarea class="form-control" readonly rows="<?php echo sizeof($output) < 10 ? sizeof($output) : 10; ?>" id="output"><?php foreach ($output as $num => $row) {
        echo "$row";
        if (sizeof($output) - 1 != $num) echo "\n";
    } ?></textarea>
<?php endif; ?>
        </div>
    </div>
