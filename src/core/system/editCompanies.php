...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['deleteCompany'])) {
        ...
        if ($conn->error) {
            ...
            echo '<div class="alert alert-over alert-success"><a href="#" data-dismiss="alert" class="close">&times;</a>' . $lang['OK_DELETE'] . '</div>';
        }
      ...
      detailLeft = '$left', detailMiddle = '$middle', detailRight = '$right', companyPostal = '$plz', uid = '$uid', companyCity = '$city' WHERE id = $cmpID");
        ...
        for ($i = 0; $i < count($_POST['default_projectNames']); $i++) {
            ...
            if (isset($_POST['default_addField_1_' . $defaultID])) {
                $field_1 = 'TRUE';
            } else {
                $field_1 = 'FALSE';
            }
            if (isset($_POST['default_addField_2_' . $defaultID])) {
                $field_2 = 'TRUE';
            } else {
                $field_2 = 'FALSE';
            }
            if (isset($_POST['default_addField_3_' . $defaultID])) {
                $field_3 = 'TRUE';
            } else {
                $field_3 = 'FALSE';
            }
    ...
    if (isset($_POST['hire']) && isset($_POST['hiring_userIDs'])) {
        ...
        if (!empty($_POST['addFinance_name']) && !empty($_POST['addFinance_num']) && $_POST['addFinance_num'] < 2999 && $_POST['addFinance_num'] > 2000) {
            ...
            $ggk = 'FALSE';
            if (isset($_POST['addFinance_booking']))
                $ggk = 'TRUE';
            $opt = 'STAT';
        ...
        $val = 'FALSE';
        if ($_POST['finance_istVersteuerer'])
            $val = 'TRUE';
        ...
        warning1, warning2, warning3) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
                ...
                while (($line = fgetcsv($file, 0, ";")) !== false) {
                    ...
                    if ($res && $res->num_rows > 0) {
                        ...
                        echo $conn->error;
                        $conn->query("DELETE FROM clientInfoData WHERE clientID = $clientID");
                        echo $conn->error;
                    } else {
                        $stmt_client->execute();
...
if ($result && ($companyRow = $result->fetch_assoc()) && in_array($companyRow['id'], $available_companies)):
    ...
    <?php echo $lang['WARNING_DELETE_COMPANY']; ?>
        ...
        <form method="post" enctype="multipart/form-data" class="page-seperated-section">
            ...
            <div class="container-fluid">
                <div class="col-sm-4">
    <?php if ($companyRow['logo']) {
        echo '<img style="max-width:350px;max-height:200px;" src="data:image/jpeg;base64,' . base64_encode($companyRow['logo']) . '"/>';
    } ?>
                </div>
                <div class="col-sm-8">
    ...
    if ($projectResult && $projectResult->num_rows > 0) {
        while ($projectRow = $projectResult->fetch_assoc()) {
            ...
            while ($extraResult && ($extraRow = $extraResult->fetch_assoc())) {
                if ($projectRow['field_' . $j] == 'TRUE') {
                    echo $extraRow['name'] . '<br>';
                } $j++;
            }
            echo '</td>';
    ...
    <?php $fieldResult = $conn->query("SELECT * FROM $companyExtraFieldsTable WHERE companyID = $cmpID");
    $i = 1; ?>
        <form method="POST" class="page-seperated-section">
            <div class="container-fluid">
                <div class="row">
                    <h4><?php echo $lang['ADDITIONAL_FIELDS']; ?>
                        <div class="page-header-button-group">
                                    <?php if ($fieldResult->num_rows < 3) {
                                        echo '<button type="button" class="btn btn-default" data-toggle="modal" data-target=".cmp-add-fields-modal" title="' . $lang['ADD'] . '"><i class="fa fa-plus"></i></button>';
                                    } ?>
            ...
            <div class="container-fluid">
                <div class="col-sm-4">
                    <form method="POST" target="_blank" action="../project/csvDownload?name=Kunden_Export">
                        <?php
                        $csv = $csv_clients . "\n";
                        $result = $conn->query("SELECT *, clientData.name AS clientName FROM clientData LEFT JOIN clientInfoData ON clientID = clientData.id WHERE companyID = $cmpID AND isSupplier = 'FALSE'");
                        echo $conn->error;
                        while ($row = $result->fetch_assoc()) {
                            $line = '';
                            $line .= $row['clientName'] . ';';
    ...
    <?php
    $csv = $csv_suppliers . "\n";
    $result = $conn->query("SELECT *, clientData.name AS clientName FROM clientData LEFT JOIN clientInfoData ON clientID = clientData.id WHERE companyID = $cmpID AND isSupplier = 'TRUE'");
    echo $conn->error;
    while ($row = $result->fetch_assoc()) {
        $line = '';
        $line .= $row['clientName'] . ';';
