...
if (isset($_POST['saveButton'])) {
    ...
    //masterpassword
    if (!empty($_POST['masterPass_new']) && !empty($_POST['masterPass_newConfirm']) && $_POST['masterPass_new'] == $_POST['masterPass_newConfirm'] && (!$masterPasswordHash || crypt($_POST['masterPass_current'], $masterPasswordHash) == $masterPasswordHash)) { //from header
        $current = '';
        $new_master = base64_encode($_POST['masterPass_new']);
        if ($masterPasswordHash) {
            $current = base64_encode($_POST['masterPass_current']);
        }
    ...
    $conn->query("UPDATE UserData SET keyCode = '' ");
    echo $conn->error;
...
<?php echo $lang['COMPLEXITY']; ?>:
        ...
        <div class="col-md-4">
            <select class="form-control" name="passwordComplexity">
                <option value="0" <?php if ($row['complexity'] === '0') {
    echo 'selected';
} ?>><?php echo $lang['COMPLEXITY_TOSTRING']['SIMPLE']; ?></option>
                <option value="1" <?php if ($row['complexity'] === '1') {
    echo 'selected';
} ?>><?php echo $lang['COMPLEXITY_TOSTRING']['MEDIUM']; ?></option>
                <option value="2" <?php if ($row['complexity'] === '2') {
    echo 'selected';
} ?>><?php echo $lang['COMPLEXITY_TOSTRING']['STRONG']; ?></option>
            </select>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-xs-2 checkbox">
            <input type="checkbox" value="person" name="enableTimechange"  <?php if ($row['expiration'] == 'TRUE') {
    echo 'checked';
} ?> /> Aktiv
        </div>
    </div>
...
<?php echo $lang['INFO_EXPIRATION']; ?>
    ...
    <div class="container-fluid">
        ...
        <div class="col-md-4">
            <input type="radio" name="enableTimechange_type" value="ALERT" <?php if ($row['expirationType'] != 'FORCE') {
    echo 'checked';
} ?>/> Optional
        </div>
        <div class="col-md-4">
            <input type="radio" name="enableTimechange_type" value="FORCE" <?php if ($row['expirationType'] == 'FORCE') {
    echo 'checked';
} ?>/> Zwingend
        </div>
    </div>
    ...
    <h4><?php echo mc_status(); ?><?php echo $lang["MASTER_PASSWORD"]; ?> <small><?php if ($masterPasswordHash): echo $lang["ENCRYPTION_ACTIVE"];
            else: echo $lang["ENCRYPTION_DEACTIVATED"];
            endif; ?></small><a role="button" data-toggle="collapse" href="#password_info_master"><i class="fa fa-info-circle"></i></a></h4>
    <br>
