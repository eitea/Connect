...
$result = $conn->query("SELECT begin, name FROM holidays WHERE begin > '$start' AND begin < '".date('Y-m-d', strtotime('+1 year'))."'"); echo $conn->error;
while($result && ($row = $result->fetch_assoc())){
    if(strpos($row['name'], '(§)')) $dates[] = "{ title: '".$row['name']."', start: '".substr($row['begin'], 0, 10)."', textColor: 'white', backgroundColor: '#999'}";
}
  ?>
