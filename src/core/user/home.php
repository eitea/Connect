            ...
            <div class="tab-pane active" id="saldo">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        if ($overTimeAdditive)
                            echo '<tr><td>' . $lang['OVERTIME_ALLOWANCE'] . '</td> <td> -' . number_format($overTimeAdditive) . ' </td></tr>';
                        if ($corrections)
                            echo "<tr><td><a data-toggle='modal' data-target='#correctionModal'>" . $lang['CORRECTION'] . ' ' . $lang['HOURS'] . '</a></td><td>' . sprintf('%+.2f', $corrections) . '</td></tr>';
...
if ($result && $result->num_rows > 0) {
    $mean_mon = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_tue = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_wed = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_thu = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_fri = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_sat = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
...
if ($result && $result->num_rows > 0) {
    $mean_sun = sprintf('%.2f', array_sum(array_column($result->fetch_all(), 0)) / $result->num_rows);
}
