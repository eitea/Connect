...
$result = $conn->query("SELECT begin, name FROM holidays WHERE ASCII(SUBSTRING(name, LENGTH(name)-1,1)) = 41 AND begin > '$start' AND begin < '".date('Y-m-d', strtotime('+1 year'))."'"); echo $conn->error;
while($result && ($row = $result->fetch_assoc())){
