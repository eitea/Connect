...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$privateBucket = $row['id'].'_privateFiles';
	...
	if($_POST['function']==="addFile"){
		try{
			...
			foreach($buckets['Buckets'] as $bucket){
				if($bucket['Name']===$privateBucket) $thereisabucket=true;
			}
			if(!$thereisabucket){
				$s3->createBucket( array('Bucket' => $privateBucket ) );
			}
		...
		$title = $_POST['title'];
		$i = 1;
		while($result = $conn->query("SELECT parent_folder FROM archive_folders WHERE folderid=$id AND userid=$userID")){
