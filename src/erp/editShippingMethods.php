...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  if(isset($_POST['save'])){
    for($i = 0; $i < count($_POST['ids']); $i++){
      ...
      if($nam){
        $conn->query("UPDATE shippingMethods SET name = '$nam' WHERE id = $x");
      } else {
    ...
    $conn->query("INSERT INTO shippingMethods (name) VALUES ('$nam')");
    if($conn->error){
      echo '<div class="alert alert-danger"><a href="#" data-dismiss="alert" class="close">&times;</a>'.$conn->error.'</div>';
    } else {
    ...
    foreach($_POST['deleteIDs'] as $x){
      $conn->query("DELETE FROM shippingMethods WHERE id = $x");
    }
  }
}
?>
  ...
  <div class="container-fluid">
    <div class="col-md-6">
      <table class="table table-hover">
        ...
        <tbody>
          <?php
          $result = $conn->query("SELECT * FROM shippingMethods");
