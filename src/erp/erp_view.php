...
<div class="page-content-fixed-130">
	...
	$filterStatus_query = ($filterings['procedures'][1] >= 0) ? 'AND ph.status = '.$filterings['procedures'][1] : "";

	//5af0481d917fe
	$result = $conn->query("SELECT id_number, ph.status, p.id, p.clientID, p.curDate, companyID, ph.id AS historyID,
		clientData.name AS clientName, companyData.name AS companyName
		FROM processHistory ph INNER JOIN proposals p ON p.id = ph.processID
