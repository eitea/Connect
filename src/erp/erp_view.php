...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete_proposal'])) {
        ...
        $conn->query("DELETE p1 FROM proposals p1 WHERE p1.id NOT IN(SELECT processID FROM processHistory WHERE processID = p1.id)");
        echo $conn->error;
        ...
        $result = $conn->query("SELECT processID, id_number FROM processHistory WHERE id = $val");
        echo $conn->error;
    ...
    SELECT clientID, status, deliveryDate, paymentMethod, shipmentType, representative, porto, portoRate, header, referenceNumrow FROM proposals WHERE id = $processID");
        echo $conn->error;
        //insert history
        $processID = $conn->insert_id;
        $conn->query("INSERT INTO processHistory(id_number, processID) VALUES('" . $row['id_number'] . "', $processID)");
        echo $conn->error;
        //insert products
...
<tbody>
    ...
    while ($result && ($row = $result->fetch_assoc())) {
        ...
        while ($row_history = $result_process->fetch_assoc()) {
            ...
            if (!in_array($current_transition, $filtered_transitions))
                continue;
            ...
            while ($rowB = $result_b->fetch_assoc()) {
                if (empty($product_placements[$current_transition][$rowB['origin']]))
                    $product_placements[$current_transition][$rowB['origin']] = 0;
                $product_placements[$current_transition][$rowB['origin']] += $rowB['quantity'];
                $balance += $rowB['quantity'] * ($rowB['price'] - $rowB['purchase']);
            }
            $transitable = false;
            if ($current_transition == 'ANG') {
                $transitable = true;
                $available_transitions = array('AUB', 'RE', 'STN');
            }
            if ($current_transition == 'AUB') {
                $transitable = true;
                $available_transitions = array('RE', 'LFS', 'STN');
            }
            if ($current_transition == 'RE') {
                $transitable = true;
                $available_transitions = array('LFS', 'GUT');
            }
            ...
            if ($transitable) { //if open positions
                if ($current_transition != 'RE') {
                    echo '<button type="button" class="btn btn-default" title="' . $lang['DELETE'] . '" data-toggle="modal" data-target=".confirm-delete-' . $i . '"><i class="fa fa-trash-o"></i></button> ';
                }
