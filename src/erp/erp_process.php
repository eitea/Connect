...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	if(isset($_POST['meta_save'])){
		...
		$meta_porto = $meta_porto_percentage = $meta_foreignOpt = 0;
		...
		if(isset($_POST['meta_curDate']) && test_Date($_POST['meta_curDate'].' 12:00:00')){
			$proposal_row['curDate'] = $meta_curDate = test_input($_POST['meta_curDate'].' 12:00:00');
		}
		if(isset($_POST['meta_deliveryDate']) && test_Date($_POST['meta_deliveryDate'].' 12:00:00')){
			$proposal_row['deliveryDate'] = $meta_deliveryDate = test_input($_POST['meta_deliveryDate'].' 12:00:00');
		}
		if(isset($_POST['meta_paymentMethod'])){
			$proposal_row['paymentMethod'] = $meta_paymentMethod = test_input($_POST['meta_paymentMethod']);
		}
		if(isset($_POST['meta_shipmentType'])){
			$proposal_row['shipmentType'] = $meta_shipmentType = test_input($_POST['meta_shipmentType']);
		}
		if(isset($_POST['meta_representative'])){
			$proposal_row['representative'] = $meta_representative = test_input($_POST['meta_representative']);
		}
		if(isset($_POST['meta_porto'])){
			$proposal_row['porto'] = $meta_porto = floatval($_POST['meta_porto']);
		}
		if(isset($_POST['meta_header'])){
			$proposal_row['header'] = $meta_header = test_input($_POST['meta_header']);
		}
		if(isset($_POST['meta_porto_percentage'])){
			$proposal_row['portoRate'] = $meta_porto_percentage = intval($_POST['meta_porto_percentage']);
		}
		if(isset($_POST['meta_foreign'])){ //5b17d623ddace
			$proposal_row['foreignOpt'] = $meta_foreignOpt = intval($_POST['meta_foreign']);
		}
		...
		representative = '$meta_representative', porto = '$meta_porto', portoRate = '$meta_porto_percentage', header = '$meta_header', foreignOpt = '$meta_foreignOpt' WHERE id = ".$proposal_row['id']);
    ...
    <div class="page-header">
        <h3><?php echo $lang['PROCESS'] .' - '. $lang['EDIT'].' <small>'.$proposal_row['id_number'].'</small>'; ?>
            <div class="page-header-button-group">
                ...
                <a href="../system/clients?custID=<?php echo $proposal_row['clientID']; ?>" class="btn btn-default" title="Kunde bearbeiten"><i class="fa fa-briefcase"></i></a>
                <div class="btn-group">
	...
	<form method="POST">
		<div class="modal-dialog modal-lg modal-content" role="document">
			...
			<div class="modal-body">
				...
				<?php
				// 5b17d623ddace
				$result = $conn->query("SELECT identifier FROM travelCountryData LEFT JOIN clientInfoData ON clientInfoData.address_Country = travelCountryData.id
					WHERE clientInfoData.clientID = ".$proposal_row['clientID'] ." LIMIT 1");
				if($result && ($row = $result->fetch_assoc()) && ($row['identifier'] == 0 || $row['identifier'] == 'AT') ):
				?>
				<div class="row">
					<div class="col-md-2">Ausland</div>
					<div class="col-md-3"><label><input type="radio" <?php if($proposal_row['foreignOpt'] == 1) echo 'checked'; ?> name="meta_foreign" value="1"> Lieferung</label> </div>
					<div class="col-md-3"><label><input type="radio" <?php if($proposal_row['foreignOpt'] == 2) echo 'checked'; ?> name="meta_foreign" value="2"> Leistung</label> </div>
					<div class="col-md-3"><label><input type="radio" <?php if($proposal_row['foreignOpt'] == 3) echo 'checked'; ?> name="meta_foreign" value="3"> Nicht EU-Ausland</label> </div>
				</div>
				<?php endif; ?>
			</div>
