...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['translate'])) {
      ...
      WHERE processHistory.processID = " . $proposal_row['id'] . " AND id_number LIKE '$transit%'");
            echo $conn->error;
            while ($row = $res->fetch_assoc()) {
            ...
            $conn->query("INSERT INTO processHistory (id_number, processID) VALUES('$num', " . $proposal_row['id'] . ")");
            echo $conn->error;
                ...
                <tbody>
                    ...
                    while ($result && ($prod_row = $result->fetch_assoc())) {
                        ...
                        if ($prod_row['name'] != 'CLEAR_TEXT' && $prod_row['name'] != 'NEW_PAGE' && $prod_row['name'] != 'PARTIAL_SUM') {
                            ...
                            if ($purchase > 0) {
                                $percentage = sprintf('%+.2f', ($sell / $purchase - 1) * 100);
                            } else {
                                $percentage = 'xx';
                            }
                            echo '<tr>';
                            ...
                            if ($partial_sum_purchase > 0) {
                                $percentage = sprintf('%+.2f', ($partial_sum_sell / $partial_sum_purchase - 1) * 100);
                            } else {
                                $percentage = 'xx';
                            }
                    ...
                    if ($sum_purchase > 0) {
                        $percentage = sprintf('%+.2f', ($sum_sell / $sum_purchase - 1) * 100);
                    } else {
                        $percentage = 'xx';
                    }
                ...
                <ul class="dropdown-menu">
                    ...
                    <li><button type="submit" class="btn btn-link" value="<?php echo $LAST_POSITION; ?>" name="add_position_page" ><?php echo $lang['NEW_PAGE']; ?></button></li>
                            ...
                            if ($article_res && $article_res->num_rows > 0) {
                                echo '<li><button type="button" class="btn btn-link" data-toggle="modal" data-target=".add_article">' . $lang['ARTICLE'] . '</button></li>';
                            }
                            ?>
                </ul>
            </div>
