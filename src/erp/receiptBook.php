...
if ($filterings['supplier'])
    $supplierQuery = 'AND supplierID = ' . $filterings['supplier'];
if ($filterings['company'])
    $companyQuery = 'AND companyID = ' . $filterings['company'];
...
<?php if (count($available_companies) > 2) echo '<th>' . $lang['COMPANY'] . '</th>'; ?> 
    ...
    WHERE companyID IN (" . implode(', ', $available_companies) . ") AND DATE(invoiceDate) >= '" . $filterings['date'][0] . "' AND DATE(invoiceDate) <= DATE('" . $filterings['date'][1] . "') $supplierQuery $companyQuery ORDER BY receiptBook.id ASC");
        echo $conn->error;
