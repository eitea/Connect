...
<table class="table table-hover">
    ...
    <tbody>
        ...
        $userQuery = $dateQuery = '';
        if ($filterings['user'])
            $userQuery = "AND UserData.id = " . $filterings['user'];
        if ($filterings['date'])
            $dateQuery = "AND DATE(payDate) >= DATE('" . $filterings['date'][0] . "') AND DATE(payDate) <=  DATE('" . $filterings['date'][1] . "') ";
...
WHERE a1.companyID = $cmpID $userQuery $dateQuery ORDER BY docNum, inDate");
        ...
        while ($result && ($row = $result->fetch_assoc())) {
            if ($row['account2'] && $row['account3']) {
                if ($row['should'] != 0)
                    $t = $row['should'] * $row['percentage'] / 100;
                if ($row['have'] != 0)
                    $t = $row['have'] * $row['percentage'] / 100;
            } else {
                if ($row['should'] != 0) {
                    $t = $row['should'] - (100 * $row['should']) / (100 + $row['percentage']);
                    $row['should'] -= $t;
                }
                if ($row['have'] != 0) {
                    $t = $row['have'] - (100 * $row['have']) / (100 + $row['percentage']);
                    $row['have'] -= $t;
                }
            }
            echo '<tr>';
