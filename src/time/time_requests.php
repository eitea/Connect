...
<table class="table table-hover">
    ...
    <tbody>
        ...
        if ($filterings['acceptance'] > -1) {
            $statusQuery = "AND userRequestsData.status = '" . $filterings['acceptance'] . "'";
        }
        if ($filterings['requestType']) {
            $typeQuery = "AND requestType = '" . $filterings['requestType'] . "'";
        }
        if ($filterings['user']) {
            $userQuery = "AND userID = " . $filterings['user'];
        }
