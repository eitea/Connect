...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['setActiveTab'])) {
        $activeTab = $_POST['setActiveTab'];
    }
    ...
    if (isset($_POST['saveTimestamp'])) {
        ...
        if ($_POST['is_open']) {
            ...
            if ($timeFin != '0000-00-00 00:00:00' && $timeFin != ':00') {
                $timeFin = carryOverAdder_Hours($timeFin, ($timeToUTC * -1));
            } else {
                $timeFin = '0000-00-00 00:00:00';
            }
        }
...
<ul class="nav nav-tabs">
    <li <?php if ($activeTab == 'home') {
                                   echo 'class="active"';
                               } ?>><a data-toggle="tab" href="#home"><?php echo $lang['VIEW_PROJECTS']; ?></a></li>
                                <?php
                                foreach ($filterings['users'] as $u) {
                                    $active = '';
                                    if ($u == $activeTab)
                                        $active = 'class="active"';
...
<div class="tab-content">
    <!-- ############################### Projects ################################### -->
    <div id="home" class="tab-pane fade <?php if ($activeTab == 'home') {
            echo 'active in';
        } ?>">
        ...
        <?php
        //quess who needs queries.
        $companyQuery = $clientQuery = $projectQuery = $userQuery = $chargedQuery = $breakQuery = $driveQuery = "";
        if ($filterings['company']) {
            $companyQuery = "AND $companyTable.id = " . $filterings['company'];
        }
        if ($filterings['client']) {
            $clientQuery = "AND $clientTable.id = " . $filterings['client'];
        }
        if ($filterings['project']) {
            $projectQuery = "AND $projectTable.id = " . $filterings['project'];
        }
        if ($filterings['users']) {
            $userQuery = "AND $userTable.id IN (" . implode(', ', $filterings['users']) . ')';
        }
        if ($filterings['bookings'][0] == '2') {
            $chargedQuery = "AND $projectBookingTable.booked = 'TRUE' ";
        } elseif ($filterings['bookings'][0] == '1') {
            $chargedQuery = " AND $projectBookingTable.booked = 'FALSE' ";
        }
        if (!$filterings['bookings'][1]) {
            $breakQuery = "AND $projectBookingTable.bookingType != 'break' ";
        } //projectID == NULL
        if (!$filterings['bookings'][2]) {
            $driveQuery = "AND $projectBookingTable.bookingType != 'drive' ";
        }
        ?>
  ...
  ORDER BY $projectBookingTable.start ASC";
                    ...
                    for ($i = 0; $i < $numRows; $i++) {
                        ...
                        if ($extraFldRes && $extraFldRes->num_rows > 0) {
                            ...
                            if ($row['extra_1']) {
                                ...
                                $csv_optionalinfo = $extraFldRow['name'] . ': ' . $row['extra_1'];
                            }
                        }
                        if ($extraFldRes && $extraFldRes->num_rows > 1) {
                            ...
                            if ($row['extra_2']) {
                                ...
                                $csv_optionalinfo .= ', ' . $extraFldRow['name'] . ': ' . $row['extra_2'];
                            }
                        }
                        if ($extraFldRes && $extraFldRes->num_rows > 2) {
                            ...
                            if ($row['extra_3']) {
                                ...
                                $csv_optionalinfo .= ', ' . $extraFldRow['name'] . ': ' . $row['extra_3'];
                            }
                        }
                        $csv .= $csv_optionalinfo . ';';
                        if ($row['exp_unit'] > 0) {
                            $expensesinfo .= $lang['QUANTITY'] . ': ' . $row['exp_unit'] . '<br>';
                            $csv_expensesinfo .= "{$lang['QUANTITY']}: {$row['exp_unit']},";
                        }
                        if ($row['exp_price'] > 0) {
                            $expensesinfo .= $lang['PRICE_STK'] . ': ' . $row['exp_price'] . '<br>';
                            $csv_expensesinfo .= "{$lang['PRICE_STK']}: {$row['exp_price']}";
                        }
                        if ($row['exp_info']) {
                            $expensesinfo .= $lang['DESCRIPTION'] . ': ' . $row['exp_info'] . '<br>';
                            $csv_expensesinfo .= ", {$lang['DESCRIPTION']}: {$row['exp_info']}";
                        }
                        $csv .= $csv_expensesinfo . ';';
                        echo "<td>";
                        if ($row['booked'] == 'FALSE') {
                            echo '<button type="button" onclick="appendModal_proj(' . $x . ', ' . $userID . ')" class="btn btn-default"><i class="fa fa-pencil"></i></button>';
                        }
                        if ($row['bookingType'] != 'break') {
                            echo "<a tabindex='0' role='button' class='btn btn-default' data-toggle='popover' data-trigger='hover' title='Stundenkonto - Stundenrate - Projektstatus' data-content='$detailInfo' data-placement='left'><i class='fa fa-info-circle'></i></a>";
                        }
                        if (!empty($interninfo)) {
                        ...
                        if (!empty($optionalinfo)) {
                            echo "<a type='button' class='btn btn-default' data-toggle='popover' data-trigger='hover' title='" . $lang['ADDITIONAL_FIELDS'] . "' data-content='$optionalinfo' data-placement='left'><i class='fa fa-question-circle'></i></a>";
                        }
    ...
    <!-- ############################### UserData ################################### -->
                ...
                <?php
                foreach ($filterings['users'] as $x):
                    if ($activeTab == $x) {
                        echo "<div id=\"$x\" class=\"tab-pane fade active in\">";
                    } else {
        ...
        LEFT JOIN $projectTable ON ($projectBookingTable.projectID = $projectTable.id) LEFT JOIN $clientTable ON ($projectTable.clientID = $clientTable.id) WHERE timestampID = ? ORDER BY end ASC");
                    ...
                    for ($i = 0; $i < $calculator->days; $i++) {
                        ...
                        if ($filterings['logs'][0] && $calculator->activity[$i] != $filterings['logs'][0])
                            continue;
                        ...
                        if ($calculator->indecesIM[$i] != 0) {
                            echo '<button type="submit" class="btn btn-default" title="' . $lang['DELETE'] . '" name="ts_remove" value="' . $calcIndexIM . '"><i class="fa fa-trash-o"></i></button>';
                        }
                        echo '</td>';
                        echo "</tr>";
                ...
                <div class="row">
                    ...
                    <div class="col-xs-6">
                        ...
                        <div class="input-group">
                            <input id="time_field" type="time" class="form-control timepicker" onkeydown='if (event.keyCode == 13)
                                        return false;' name="start" placeholder="00:00"/>
                            <span class="input-group-addon"> - </span>
                            <input type="time" class="form-control timepicker" onkeydown='if (event.keyCode == 13)
                                        return false;' name="end" placeholder="00:00"/>
                        </div>
                    </div>
                </div>
            </div>
...
<?php echo $lang['DATATABLES_LANG_OPTIONS']; ?>
                                    ...
                                    setTimeout(function () {
                                        ...
                                        $('#projectTable').trigger('column-reorder.dt');
                                    }, 500);
                                });
</script>
<!-- /BODY -->
