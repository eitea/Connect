    ...
    */
    ...
    var remember_state = {
        ...
        _defaultNoticeDialog: function() {
            ...
            .html('Letzte Eingabe Wiederherstellen? <a href="#" class="refill btn btn-link btn-sm">Ja</a> <span href="#" class="cancel btn btn-link btn-sm">Nein</span> ');
        },
        noticeDialog: null,
        ignore: null,
        ...
        restoreState: function(e) {
            ...
            for (var i in data) {
                ...
                if($e.is(".dont-remember") || $e.attr('type') == 'hidden' || $e.is(":hidden")) continue;
                if ($e.is(":radio")) {
                    ...
                    $e.find("[value=\"" + data[i].value + "\"]").prop("selected", true).change();
                    //sleep(250);
                } else if ($e.is("textarea[name=\"" + data[i].name + "\"]")){
                    try {
                        tinymce.get(data[i].name).setContent(data[i].value);
                    } catch(err) {
                        $e.val(data[i].value).trigger('change');
                    }
                } else {
                    $e.val(data[i].value);
                }
        ...
        bindNoticeDialog: function() {
            ...
            this.noticeDialog.on("click." + this.name, "a", $.proxy(this.restoreState, this));
            this.noticeDialog.on("click." + this.name, "span", $.proxy(this.cancelNotice, this));
        },
        setName: function() {
        ...
        createNoticeDialog: function() {
            if (localStorage[this.objName]) {
                ...
                // }
                div = this.$el;
                if(this.$el.find('.remember_state:first').length){
                    div = this.$el.find('.remember_state:first');
                }
                this.noticeDialog.prependTo(div).show();
            }
        },
